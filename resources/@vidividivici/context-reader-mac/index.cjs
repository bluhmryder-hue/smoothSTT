...
function getWindowContext() {
  ...
  return new Promise((resolve) => {
    execFile(getBinaryPath(), [], { timeout: 6000 }, (err, stdout) => {
      if (err) {
        console.error("Context Reader execution failed:", err);
        resolve(null);
        return;
      }
      try {
        ...
        if (data && data.error) {
          console.error("Context Reader returned an error:", data.error);
          resolve(null);
          return;
        }
        resolve(data);
      } catch (parseError) {
        console.error("Failed to parse Context Reader output:", parseError, "Output:", stdout);
        resolve(null);
      }
    });
  });
